...
int batch_sort_function ( struct Batch *b0, struct Batch *b1 )
{
  N32 t0 = b0->total; 
  N32 t1 = b1->total; 
...
Z32 process_batch ( Search s, FILE *f, N32 bn )
{
  ...
  while ( fgets(word, W_LENGTH_MAX, f) && strcmp (word, "\n") ) 
    {
      word[strlen(word)-1] = '\000'; /* chop */
  ...
  if ( s->debug )
    {
      sprintf (lmsg, "read %d words, %d/%d occurences/blocks total", b->howmany, b->total, b->blockmap_l); 
