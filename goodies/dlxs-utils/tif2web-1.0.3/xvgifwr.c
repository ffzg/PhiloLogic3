...
/*****************************************************************
 * Stuff that was in xv.h that we need.
 */
...
static void compress(int, FILE *, byte *, int, int);
...
/*************************************************************/
int WriteGIF(fp, pic, w, h, rmap, gmap, bmap, numcols, colorstyle, trans)
FILE *fp;
byte *pic;
int   w,h;
byte *rmap, *gmap, *bmap;
...
int   trans;
{
  ...
  /* if we want a transparent background, put out an expansion area */
  if (trans)
  {
      char gce[5];

      /* put out a graphic extension section */
      fputs("\041\371\004",fp);
      memset(gce, sizeof(gce),0);
      gce[0] |= 1;		/* set transparent color flag */
      gce[3] = Background;	/* make background color transparent */
      fwrite(gce,sizeof(gce),1,fp);
  }

  /* put out the image data section */
  ...
  compress(InitCodeSize+1, fp, pic, w*h, BitsPerPixel > 1);
...
/********************************************************/
static void compress(init_bits, outfile, data, len, bpp)
int   init_bits;
FILE *outfile;
byte *data;
int   len;
int   bpp;		/* 1 byte per pixel... (vs. 1 bit) */
{
  register long fcode;
  register int i = 0;
  register int d;
